...
def split_audio(audio_path, split_duration):
    ...
    for i in range(0, len(sound)-1000, split_duration * 1000):
        segment = sound[i:i+split_duration*1000]
        segment_duration = str((i/(1000))) + '-' + \
            str((i+split_duration*1000)/(1000))
        print(
            f"./data/audio/segments/{file_name}/{file_name}_{segment_duration}.mp3 was created!")
        segment.export(
            f"./data/prod/audio/segments/{file_name}/{file_name}_{segment_duration}.mp3", format="mp3")
